...
acha_overlay_from_rds <- function(data_prefix) {
    ...
    pd <- as_draws_df(m, variable = c('b_Intercept','b_c_Time','b_quad_c_Time')) %>% as.data.frame()
    names(pd)[1:3] <- c('b0','b1','b2')
    tt <- seq(0, 10.5, by = .125)
    out <- data.frame()
    for(r in 1:nrow(pd)) {
        perc <- logits_to_prob(pd$b0[r] + tt*pd$b1[r] + tt^2*pd$b2[r]) * 100
...
for(yvar in names(yvars)){
    ...
    post_df <- as_draws_df(model, variable = c('b_Intercept', 'b_c_Time', 'b_quad_c_Time')) %>% as.data.frame()
...
for(var in base_vars) {
    if(var %in% names(ncha_yvars)){
        ...
        acha_plot_df <- acha_overlay_from_rds(ncha_yvars[[var]]['data_prefix'] %>% unname())
